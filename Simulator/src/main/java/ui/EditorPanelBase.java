...
public abstract class EditorPanelBase extends JPanel {
	...
	public final void setModel(final EditModel newModel) {
		mutexGetSetModel.acquireUninterruptibly();
		try {
			if (newModel==null) model=new EditModel(); else model=newModel.clone();
	...
	public final String saveModel(File file) {
		...
		if (file==null) {
			file=XMLTools.showSaveDialog(getParent(),SAVE_MODEL);
	 ...
	 */
	protected final JPanel addTab(final String name, final Icon icon) {
		if (tabs==null) {
